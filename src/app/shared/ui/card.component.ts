    ...
    <div class="surface-panel overflow-hidden group">
      @if (title()) {
        <div class="px-7 py-6 border-b border-[color:var(--panel-outline)] flex items-start justify-between gap-3">
          <div>
            <h2 class="text-2xl font-bold text-[color:var(--text-primary)] tracking-tight font-display">
              {{ title() }}
            </h2>
            @if (subtitle()) {
              <p class="text-sm text-[color:var(--text-muted)] mt-1 font-medium">{{ subtitle() }}</p>
            }
          </div>
